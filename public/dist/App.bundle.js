...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // this.heart is any element with the name heart in form
    (0, _bling.$)('.heart-count').textContent = res.data.hearts.length;
    if (isHearted) {
      var heartFloatAnimationClass = 'heart__button--float';
      _this.heart.classList.add(heartFloatAnimationClass);
      setTimeout(function () {
        return _this.heart.classList.remove(heartFloatAnimationClass);
      }, 2500);
    }
  }).catch(console.error);
}
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // then zoom the map to fit all the maerkers perfectly
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    ...
    } else if (keyCode === 13 && current && current.href) {
